...
Function FE_verteilen()
    ...
    If MsgBox("Aktuelle Version verteilen?", vbYesNo) = vbYes Then
        ...
        'Produktiv verbinden (Lokal) -> Funktioniert nicht bei Verknüpfung!!!!
        'switchConnectAcc PfadProdLokal & Backend
    ...
    Select Case tabelle
        ...
        Case 1
            ...
            If rst.RecordCount > 1 Then
                ...
                sql = "UPDATE " & ZUORDNUNG & " SET PosNr = Null WHERE " & Criteria
                CurrentDb.Execute sql
        ...
        Case 2
            ...
            If rst.RecordCount > 1 Then
                ...
                sql = "UPDATE " & PLANUNG & " SET PosNr = Null WHERE " & Criteria
                CurrentDb.Execute sql
...
Dim tbl         As String
    ...
    If iAnstArt = 13 Then
        sql = sql & " OR Anstellungsart_ID = 5 OR Anstellungsart_ID = 3 OR Anstellungsart_ID = 4)"
    ElseIf iAnstArt = 5 Then
        sql = sql & " OR Anstellungsart_ID = 5 OR Anstellungsart_ID = 4)" 'Mini + Midi Jobber
...
'Die Felder MA_Start und MA_Ende enthalten nur die Uhrzeit ohne Datum!
...
Public Function AddWhereAndOrderByToSQL(strSQL As String, Optional strFilter As String, Optional strOrderBy As String) As String
...
Function qr_erstellen(Optional iMA_ID As Long, Optional iZuo_ID As Long, Optional iText As String) As String
...
Dim text2       As String
    ...
    If Not IsInitial(iText) Then
        Text = iText
        text2 = CleanFileName(iText)
        data = iText
        
    ElseIf IsInitial(iMA_ID) Then
        MA_ID = iMA_ID
        data = MA_ID
        Name = Nz(TLookup("Nachname", MASTAMM, "ID=" & MA_ID), "") & " " & Nz(TLookup("Vorname", MASTAMM, "ID=" & MA_ID), "")
        Text = Name
        text2 = CleanFileName(Name)
        
    ElseIf IsInitial(iZuo_ID) Then
        ZUO_ID = iZuo_ID
...
Set rs = CurrentDb.OpenRecordset("SELECT DISTINCT MA_ID, VA_ID FROM " & ZUORDNUNG & " WHERE VA_ID in (SELECT ID FROM " & AUFTRAGSTAMM & " WHERE sub_send = true)")
...
Do While Not rs.EOF
    ...
    If TLookup("istSubunternehmer", MASTAMM, "ID=" & MA_ID) = True Then _
        CurrentDb.Execute ("INSERT INTO [ztbl_VA_SUB] (MA_ID,VA_ID,Datum) VALUES (" & MA_ID & "," & VA_ID & "," & DatumSQL(Now) & ")")
    rs.MoveNext
...
'Wert Dienstkleidung berechnen
Function calc_DienstKL(MA_ID) As Double
...
Dim Wert As Double
    ...
    If Not IsNull(MA_ID) Then
        ...
        Do While Not rs.EOF
            If Not IsNull(rs.fields("Kautionsbetrag")) Then Wert = Wert + (rs.fields("Anzahl") * rs.fields("Kautionsbetrag"))
            rs.MoveNext
        Loop
        rs.Close
        Set rs = Nothing
...
'TEST: ?transfer_DP_webDB(56,"C:\Consys\Dokumente\Auftrag\Zusage\DP_56.pdf")
Function transfer_DP_webDB(MA_ID As Long, file As String) As String
...
Dim ID                  As Long
    ...
    conn.connectionString = "CONNECT_MARIADB"
    conn.Open
    
    sql = "SELECT id FROM " & tbl & " WHERE mitarbeiter_id=" & MA_ID
    ...
    'Datei einlesen
    arr = zBase64Encoder.readFileToArray(file)
    b64 = zBase64Encoder.ConvertToBase64String(arr)
    
    If ID = 0 Then
        sql = "INSERT INTO " & tbl & " (mitarbeiter_id, pdf_b64_encrypted) VALUES (ID, '" & b64 & "')"
    Else
        sql = "UPDATE " & tbl & " SET pdf_b64_encrypted = '" & b64 & "' WHERE id=" & ID
    End If
