...
' Version 3.1 - Standalone EXE Ansatz mit API-Server Auto-Start
' =====================================================
...
' Öffnet das Mitarbeiterstammblatt als HTML
Public Sub OpenMitarbeiterstammHTML(Optional MA_ID As Long = 0)
    ...
    If Not EnsureAPIServerRunning() Then
        If MsgBox("API-Server nicht verfügbar. Formular trotzdem öffnen?", vbYesNo + vbQuestion) = vbNo Then
            Exit Sub
        End If
    End If
...
' Öffnet die Auftragsverwaltung als HTML
...
' Öffnet das Kundenstammblatt als HTML
...
Public Sub OpenWebViewForm(htmlPath As String, title As String, width As Long, height As Long, Optional jsonData As String = "{}")
    ...
    ' Prüfe ob HTML existiert
    If Dir(htmlPath) = "" Then
        MsgBox "HTML-Datei nicht gefunden:" & vbCrLf & htmlPath, vbCritical, "Fehler"
        Exit Sub
    End If
    
    ' JSON-Daten in temporäre Datei schreiben (für komplexe Daten)
    ...
    ' Daten nur anhängen wenn nicht zu lang
    If Left(jsonData, 1) <> "@" And Len(jsonData) > 2 Then
        ' Einfache Daten direkt übergeben
        ' Escaping für Kommandozeile
...
' =====================================================
' DATEN-SERVICE: Lädt Daten aus Access-Tabellen
' =====================================================

' Lädt Mitarbeiter-Daten als JSON
