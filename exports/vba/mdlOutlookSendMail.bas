Attribute VB_Name = "mdlOutlookSendMail"
...
'    Body = Replace(Body, "»", "")         'Mist raus
    ...
    CurrentDb.Execute "INSERT INTO tbl_Log_eMail_Sent(SendDate,Absender,Betreff,MailText,BCC,IstHTML,Attachment)" & _
        " VALUES (" & DatumUhrzeitSQL(Now) & ",'" & Environ("UserName") & "','" & Subject & "','zCreatePlainMail','" & SendToBCC & "'," & IstHTML & "," & IIf(IsInitial(myattach(0)), 0, -1) & ")"
