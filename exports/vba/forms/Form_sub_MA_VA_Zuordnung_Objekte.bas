...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_sub_MA_VA_Zuordnung_Objekte"
...
'Form_frm_DP_Dienstplan_Objekt.Bookmark = GL_strBookmark_Object
...
Dim iVerfueg As Long
    ...
    If VAStartID_Buffer <> Me.VAStart_ID Then
        VAStartID_Buffer = Me.VAStart_ID
...
Me.Parent!PKW_Anzahl = Null
i = TCount("*", "tbl_MA_VA_Zuordnung", "PKW > 0 AND VA_ID = " & Me.Parent!ID & " AND VADatum_ID = " & Me.Parent!cboVADatum)
If i > 0 Then
    Me.Parent!PKW_Anzahl = i
Else
    Me.Parent!PKW_Anzahl = 0
    Me.Parent!PKW_Anzahl.Visible = False
End If
DoEvents
End Sub
