Attribute VB_Name = "zmd_Listen"
Option Compare Database
Option Explicit

' ... Mit KI-Mist, letzter sauberer Stand unten ....
...
''...Funktonierender Stand ohne KI-Mist...
'Function Stundenliste_erstellen(VA_ID As Long, Optional MA_ID As Long, Optional kun_ID As Long)
'
''ESS Sonderlocken:
''Kun_id 20730 ESS 1 Bereichswachen
...
'            If kun_ID = 20730 Or kun_ID = 20760 Or kun_ID = 20761 Then
'                .Range("L" & i).NumberFormat = "@"
'                .Range("L" & i) = rs.fields("Position")
'                If UBound(Split(Nz(rs.fields("Bemerkungen"), " "), " ")) > 0 Then
'                    .Range("M" & i) = Split(Nz(rs.fields("Bemerkungen"), " "), " ")(0) & Split(Nz(rs.fields("Bemerkungen"), " "), " ")(1)
'                Else
'                    .Range("M" & i) = Nz(rs.fields("Bemerkungen"), "")
'                End If
'                If UBound(Split(Nz(rs.fields("Bemerkungen"), " "), " ")) > 1 Then .Range("N" & i) = Split(Nz(rs.fields("Bemerkungen"), " "), " ")(2)
'                '.Range("M" & i) = Split(Nz(rs.fields("Bemerkungen"), "-"), "-")(0)
'                '.Range("N" & i) = Split(Nz(rs.fields("Bemerkungen"), "-"), "-")(1)
'            End If
'            rs.MoveNext
'            i = i + 1
'
...
'        .SortFields.Add key:=xlWs.Range("D4:D" & Count + 1), SortOn:=0, Order:=1, DataOption:=0
'        .SortFields.Add key:=xlWs.Range("B4:B" & Count + 1), SortOn:=0, Order:=1, DataOption:=0
'        .SetRange xlWs.Range("A4:L" & Count + 1)
'        .Header = 1
'        .MatchCase = False
'        .Orientation = 1
'        .SortMethod = 1
...
'            .Range("N:N").EntireColumn.AutoFit
'            .Range("F" & Count + 5) = "SVS"
'            .Range("G" & Count + 5) = TLookup("StdPreis", SPREISE, "kun_ID=" & kun_ID & " AND Preisart_ID=1")                            'Normal
'            .Range("H" & Count + 5) = .Range("G" & Count + 5) + TLookup("StdPreis", SPREISE, "kun_ID=" & kun_ID & " AND Preisart_ID=11") 'Nacht
'            .Range("I" & Count + 5) = .Range("G" & Count + 5) + TLookup("StdPreis", SPREISE, "kun_ID=" & kun_ID & " AND Preisart_ID=12") 'Sonntag
'            .Range("J" & Count + 5) = .Range("G" & Count + 5) + TLookup("StdPreis", SPREISE, "kun_ID=" & kun_ID & " AND Preisart_ID=13") 'Feiertag
'            .Range("F" & Count + 6) = "Betrag"
'            .Range("G" & Count + 6) = .Range("G" & Count + 3) * .Range("G" & Count + 5)
'            .Range("H" & Count + 6) = .Range("H" & Count + 3) * .Range("H" & Count + 5)
'            .Range("I" & Count + 6) = .Range("I" & Count + 3) * .Range("I" & Count + 5)
...
'            .Range("F" & Count + 9) = "Ges. Brutto"
'            .Range("G" & Count + 9) = .Range("G" & Count + 7) + .Range("G" & Count + 8)
'            .Range("G" & Count + 5).NumberFormat = "#,##0.00 €"
'            .Range("H" & Count + 5).NumberFormat = "#,##0.00 €"
'            .Range("I" & Count + 5).NumberFormat = "#,##0.00 €"
'            .Range("J" & Count + 5).NumberFormat = "#,##0.00 €"
'            .Range("G" & Count + 6).NumberFormat = "#,##0.00 €"
'            .Range("H" & Count + 6).NumberFormat = "#,##0.00 €"
'            .Range("I" & Count + 6).NumberFormat = "#,##0.00 €"
'            .Range("J" & Count + 6).NumberFormat = "#,##0.00 €"
'            .Range("F" & Count + 7).Font.Underline = 2 'xlUnderlineStyleSingle
'            .Range("G" & Count + 7).NumberFormat = "#,##0.00 €"
'            .Range("G" & Count + 7).Font.Underline = 2 'xlUnderlineStyleSingle
'            .Range("G" & Count + 8).NumberFormat = "#,##0.00 €"
'            .Range("F" & Count + 9).Font.Underline = 2 'xlUnderlineStyleSingle
'            .Range("G" & Count + 9).Font.Underline = 2 'xlUnderlineStyleSingle
'            .Range("G" & Count + 9).NumberFormat = "#,##0.00 €"
'            .Range("K" & Count + 8) = "Geprüft ESS:"
'            .Range("K" & Count + 8).Font.color = vbRed
'
'        Else
'            .Range("L" & Count + 3).NumberFormat = "#,##0.00 €"
'
'        End If
'
