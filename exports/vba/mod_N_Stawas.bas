Attribute VB_Name = "mod_N_Stawas"
...
Private Function Email_Import_Zu_Excel(ByRef xlApp As Object) As String
    ...
    If mail Is Nothing Then
        MsgBox "Bitte eine E-Mail in Outlook oeffnen oder markieren.", vbExclamation
        Email_Import_Zu_Excel = ""
        Exit Function
    End If
...
ContinueLoop:
    ...
    If lastOut < 2 Then
        MsgBox "Keine gueltigen Datensaetze fuer die rechte Tabelle vorhanden.", vbInformation
        Exit Sub
    End If
...
Private Function SucheBestehendenAuftrag(basisName As String, DatumVon As Date, DatumBis As Date) As Long
    ...
    If Not rs.EOF Then
        Debug.Print ">>> EXAKTE UEBEREINSTIMMUNG GEFUNDEN <<<"
    ...
    If basisOhneTimestamp <> suchName Then
        ...
        If Not rs.EOF Then
            Debug.Print ">>> LIKE-UEBEREINSTIMMUNG GEFUNDEN <<<"
            Debug.Print "ID: " & rs!ID & " | Auftrag: " & rs!Auftrag
            SucheBestehendenAuftrag = rs!ID
            rs.Close
            Set rs = Nothing
    ...
    If Len(erstesWort) >= 5 Then
        ...
        If Not rs.EOF Then
            Debug.Print ">>> AEHNLICHE AUFTRAEGE GEFUNDEN <<<"
            Do While Not rs.EOF
...
Private Sub AktualisiereDatumsBereich(ByVal VA_ID As Long, ByVal neuDatumVon As Date, ByVal neuDatumBis As Date)
    ...
    If Not rs.EOF Then
        ...
        Debug.Print "=== DATUMS-BEREICH PRUEFUNG ==="
        ...
        If geaendert Then
            ...
            Debug.Print "OK Auftragsstamm aktualisiert!"
            Debug.Print "FINAL: " & Format(finalDatumVon, "dd.mm.yyyy") & " - " & Format(finalDatumBis, "dd.mm.yyyy") & " (" & anzTage & " Tage)"
        Else
            Debug.Print "Keine Aenderung noetig (neue Daten innerhalb bestehender Zeitspanne)"
        End If
        Debug.Print "=============================="
    End If
...
Private Sub Excel_Auftrag_Anlegen(xlApp As Object)
    ...
    Dim strMVA_Start As String, strMVA_Ende As String
    
    Debug.Print "=== Excel_Auftrag_Anlegen START ==="
    
    Set xlWb = xlApp.ActiveWorkbook
    If xlWb Is Nothing Then
        MsgBox "Keine Excel-Arbeitsmappe geoeffnet!", vbExclamation
        Exit Sub
    End If
    ...
    If Not validDatesFound Then
        MsgBox "Keine gueltigen Datumswerte!", vbExclamation
        Exit Sub
    End If
    ...
    For i = startRow To lastRow
        ...
        strMVA_Start = "#" & Month(Datum) & "/" & Day(Datum) & "/" & Year(Datum) & " " & Format(Hour(Beginn), "00") & ":" & Format(minute(Beginn), "00") & ":" & Format(Second(Beginn), "00") & "#"
        strMVA_Ende = "#" & Month(Datum) & "/" & Day(Datum) & "/" & Year(Datum) & " " & Format(Hour(Ende), "00") & ":" & Format(minute(Ende), "00") & ":" & Format(Second(Ende), "00") & "#"
        ...
        Else
            rs.Close
            Set rs = Nothing
            sqlStr = "INSERT INTO tbl_VA_Start (VA_ID, VADatum_ID, VADatum, MA_Anzahl, VA_Start, VA_Ende, MVA_Start, MVA_Ende, Bemerkungen) VALUES (" & _
                     newVA_ID & ", " & vaDatumID & ", #" & Month(Datum) & "/" & Day(Datum) & "/" & Year(Datum) & "#, 1, " & _
                     strStartZeit & ", " & strEndeZeit & ", " & strMVA_Start & ", " & strMVA_Ende & ", '" & Replace(stand, "'", "''") & "')"
            db.Execute sqlStr, dbFailOnError
...
NextRow:
    ...
    If istNachbestellung Then
        MsgBox "Zusaetzliche Schichten wurden hinzugefuegt", vbInformation
    Else
        ...
        meldung = "Neuer Auftrag erstellt!" & vbCrLf & vbCrLf & _
                  ...
                  "Schichten MERGE: " & mergeCounter & vbCrLf & _
                  "Uebersprungen: " & skipCounter
