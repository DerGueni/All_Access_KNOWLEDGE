...
(function() {
    ...
    // ABER: Nicht initialisieren wenn WebView2 erkannt wurde (Bridge übernimmt)
    function shouldInit() {
        // Skip wenn WebView2 vorhanden
        if (window.chrome && window.chrome.webview) {
            console.log('[API-Lifecycle] WebView2 erkannt - überspringe API-Server-Check');
            return false;
        }
        // Skip wenn Bridge das Flag gesetzt hat
        if (window._skipApiLifecycle) {
            console.log('[API-Lifecycle] Skip-Flag gesetzt - überspringe API-Server-Check');
            return false;
        }
        return true;
    }
