...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof Bridge !== 'undefined') {
        ...
        Bridge.on('onSaveComplete', handleSaveComplete);
            ...
            if (!isWebView2) { console.log('Browser-Modus KD'); setTimeout(loadDemoData, 100);
...
function loadDemoData() {
    console.log('Lade Kunden-Echtdaten...');
    
    if (typeof KUNDEN_ECHTDATEN !== 'undefined' && KUNDEN_ECHTDATEN.length > 0) {
        console.log('KUNDEN_ECHTDATEN gefunden:', KUNDEN_ECHTDATEN.length, 'Kunden');
        
        const data = {
            type: 'kunde',
            kunde: KUNDEN_ECHTDATEN[0],
            alleKunden: KUNDEN_ECHTDATEN
        };
        
        handleDataReceived(data);
    } else {
        console.warn('Keine Kunden-Echtdaten gefunden');
    }
},
...
function fillStammdaten(kd) {
    ...
    // Stammdaten-Felder
    setElementChecked('chkAktiv', kd.istAktiv);
    setElementValue('txtKuerzel', kd.kuerzel);
    setElementValue('dispFirma', kd.firma);
    setElementValue('txtStrasse', kd.strasse);
    setElementValue('txtPLZ', kd.plz);
    setElementValue('txtOrt', kd.ort);
    setElementValue('selLand', kd.land || 'DE');
    setElementValue('txtTelefon', kd.telefon);
    setElementValue('txtMobil', kd.mobil);
    setElementValue('txtEmail', kd.email);
    setElementValue('txtHomepage', kd.homepage);
    
    // Bankdaten
    setElementValue('txtKreditinstitut', kd.kreditinstitut);
    setElementValue('txtIBAN', kd.iban);
    setElementValue('txtBIC', kd.bic);
    setElementValue('txtUstIdNr', kd.ustIdNr);
    
    // Zahlungsbedingungen
    setElementValue('selZahlbed', kd.zahlbed);
...
function collectFormData() {
    return {
        ...
        kuerzel: getElementValue('txtKuerzel'),
        strasse: getElementValue('txtStrasse'),
        plz: getElementValue('txtPLZ'),
        ort: getElementValue('txtOrt'),
        land: getElementValue('Land'),
        telefon: getElementValue('txtTelefon'),
        mobil: getElementValue('txtMobil'),
        email: getElementValue('txtEmail'),
        homepage: getElementValue('txtHomepage'),
        iban: getElementValue('txtIBAN'),
        bic: getElementValue('txtBIC'),
        ustIdNr: getElementValue('txtUstIdNr'),
        istAktiv: getElementChecked('chkAktiv')
    };
}
