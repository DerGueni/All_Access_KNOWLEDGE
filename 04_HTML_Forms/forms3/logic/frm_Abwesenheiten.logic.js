...
function renderTable() {
    ...
    elements.tbody.innerHTML = state.records.map((rec, idx) => {
        // API gibt DatVon/DatBis zurueck, Legacy verwendet vonDat/bisDat
        const vonDat = (rec.DatVon || rec.vonDat) ? new Date(rec.DatVon || rec.vonDat).toLocaleDateString('de-DE') : '-';
        const bisDat = (rec.DatBis || rec.bisDat) ? new Date(rec.DatBis || rec.bisDat).toLocaleDateString('de-DE') : '-';
...
function displayRecord(rec) {
    ...
    // API gibt DatVon/DatBis zurueck, Legacy verwendet vonDat/bisDat
    const vonDat = rec.DatVon || rec.vonDat || '';
    const bisDat = rec.DatBis || rec.bisDat || '';
    elements.NV_VonDat.value = vonDat ? vonDat.split('T')[0] : '';
    elements.NV_BisDat.value = bisDat ? bisDat.split('T')[0] : '';
...
async function saveRecord() {
    ...
    if (!ma_id) {
        if (typeof Toast !== 'undefined') Toast.warning('Bitte Mitarbeiter auswaehlen');
        else alert('Bitte Mitarbeiter auswaehlen');
        elements.NV_MA_ID.focus();
        return;
    }

    if (!vonDat || !bisDat) {
        if (typeof Toast !== 'undefined') Toast.warning('Bitte Zeitraum angeben');
        else alert('Bitte Zeitraum angeben');
        return;
    }
...
async function deleteRecord() {
    const id = elements.NV_ID.value;
    if (!id) {
        if (typeof Toast !== 'undefined') Toast.warning('Kein Datensatz ausgewaehlt');
        else alert('Kein Datensatz ausgewaehlt');
        return;
    }
    ...
    try {
        ...
        setStatus('Fehler: ' + error.message);
        if (typeof Toast !== 'undefined') Toast.error('Fehler beim Loeschen: ' + error.message);
        else alert('Fehler beim Loeschen: ' + error.message);
    }
}
