...
async function loadMitarbeiter() {
    ...
    try {
        ...
        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
            ...
            // Fallback: Dummy-Daten für Entwicklung
            state.mitarbeiterList = [];
            renderMitarbeiterDropdown();
            updateStatus('WebView2 Bridge nicht verfügbar');
        }
    } catch (error) {
...
async function loadAbwesenheitsgruende() {
    showLoading();
    try {
        // WebView2 Bridge: Abwesenheitsgründe laden
        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
            Bridge.sendEvent('loadAbwesenheitsgruende', {});
        } else {
...
function handleBridgeData(data) {
    console.log('[Abwesenheitsplanung] Bridge Data empfangen:', data);

    if (data.mitarbeiter) {
        state.mitarbeiterList = (data.mitarbeiter || []).filter(ma =>
    ...
    if (data.abwesenheitsgruende) {
        state.abwesenheitsgruende = data.abwesenheitsgruende || [];
        renderAbwesenheitsgruendeDropdown();
    }

    if (data.saveSuccess) {
        showToast('Abwesenheiten erfolgreich gespeichert', 'success');
