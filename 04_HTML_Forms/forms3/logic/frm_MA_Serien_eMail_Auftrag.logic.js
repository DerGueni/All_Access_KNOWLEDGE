...
async function loadEmailVorlagen() {
    try {
        ...
        if (data.success) {
            ...
            console.error('[MA_Serien_eMail_Auftrag] Fehler beim Laden der Vorlagen:', data.error);
        }
    } catch (error) {
        console.error('[MA_Serien_eMail_Auftrag] Fehler beim Laden der Vorlagen:', error);
...
function onVorlageChange() {
    ...
    if (vorlage) {
        ...
        console.log('[MA_Serien_eMail_Auftrag] Vorlage geladen:', vorlage.name);
    }
}
