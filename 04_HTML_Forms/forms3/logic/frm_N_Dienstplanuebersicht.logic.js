...
async function loadEinsaetze() {
    ...
    try {
        ...
        // WebView2 Bridge: Einsatztage laden
        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
...
async function showDetail(einsatz) {
    ...
    try {
        // WebView2 Bridge: Zuordnungen laden
        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
            Bridge.sendEvent('loadZuordnungenDetail', { vas_id: einsatz.ID });
        } else {
            document.getElementById('detailMAListe').innerHTML = '<li>Bridge nicht verfügbar</li>';
        }
...
function handleBridgeData(data) {
    console.log('[Dienstplanübersicht] Bridge Data empfangen:', data);
    ...
    if (data.zuordnungenDetail) {
        const maList = document.getElementById('detailMAListe');
        const mitarbeiter = data.zuordnungenDetail || [];
