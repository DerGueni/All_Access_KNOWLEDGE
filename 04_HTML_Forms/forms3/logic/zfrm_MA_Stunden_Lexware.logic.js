...
if (typeof Bridge !== 'undefined' && Bridge.on) {
    ...
    console.log('[Lexware] Bridge Event-Handler registriert');
}

function handleBridgeData(data) {
    console.log('[Lexware] Bridge Data empfangen:', data);

    if (data.mitarbeiter) {
        state.mitarbeiterList = data.mitarbeiter || [];
    ...
    if (data.stunden) {
        state.stundenData = data.stunden || [];
        renderStundenTable();
        updateStatusBar();
    ...
    if (data.stundenAbgleich) {
        state.abgleichData = data.stundenAbgleich || [];
        renderAbgleichTable();
        console.log(`[Lexware] ${state.abgleichData.length} Abgleicheinträge geladen`);
        showLoading(false);
    }

    if (data.zeitkontenFehler) {
        state.fehlerData = data.zeitkontenFehler || [];
        renderFehlerTable();
    ...
    if (data.error) {
        console.error('[Lexware] Bridge Fehler:', data.error);
...
// ============================================
// DATEN LADEN (VIA BRIDGE)
// ============================================
async function loadMitarbeiter() {
    try {
        showLoading(true);
        console.log('[Lexware] Lade Mitarbeiter via Bridge...');

        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
            Bridge.sendEvent('loadMitarbeiter', { aktiv: true });
        } else {
            console.error('[Lexware] Bridge nicht verfügbar');
            showNotification('Bridge nicht verfügbar', 'error');
            showLoading(false);
        }
    } catch (e) {
...
async function loadStundenData() {
    try {
        showLoading(true);
        console.log('[Lexware] Lade Stunden via Bridge...');

        // Parameter zusammenstellen
        const params = {};
        if (state.zeitraumVon) params.von = state.zeitraumVon;
        if (state.zeitraumBis) params.bis = state.zeitraumBis;
        if (state.selectedMA) params.ma_id = state.selectedMA;
        if (state.anstellungsart) params.anstellungsart = state.anstellungsart;

        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
            Bridge.sendEvent('loadStunden', params);
        } else {
            console.error('[Lexware] Bridge nicht verfügbar');
            showNotification('Bridge nicht verfügbar', 'error');
            showLoading(false);
...
async function loadAbgleichData() {
    try {
        showLoading(true);
        console.log('[Lexware] Lade Abgleich via Bridge...');
        ...
        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
            Bridge.sendEvent('loadStundenAbgleich', params);
        } else {
            console.error('[Lexware] Bridge nicht verfügbar');
            showNotification('Bridge nicht verfügbar', 'error');
            showLoading(false);
...
async function loadFehlerData() {
    try {
        showLoading(true);
        console.log('[Lexware] Lade Fehler via Bridge...');

        if (typeof Bridge !== 'undefined' && Bridge.sendEvent) {
            Bridge.sendEvent('loadZeitkontenFehler', {});
        } else {
            console.error('[Lexware] Bridge nicht verfügbar');
            showNotification('Bridge nicht verfügbar', 'error');
            showLoading(false);
