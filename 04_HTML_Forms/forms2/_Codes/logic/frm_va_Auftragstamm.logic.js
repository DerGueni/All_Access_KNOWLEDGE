...
function initButtons() {
    ...
    bindButtonAliases(['btnErster', 'Befehl43'], () => gotoRecord(0));
    bindButtonAliases(['btnVorheriger', 'Befehl41'], () => gotoRecord(state.recordIndex - 1));
    bindButtonAliases(['btnNaechster', 'Befehl40'], () => gotoRecord(state.recordIndex + 1));
    bindButtonAliases(['btnLetzter', 'btn_letzer_Datensatz'], () => gotoRecord(state.records.length - 1));
    bindButtonAliases(['btnRueck', 'btn_rueck'], undoChanges);

    // Datum-Navigation
    bindButtonAliases(['btnDatumLeft'], () => navigateVADatum(-1));
    bindButtonAliases(['btnDatumRight'], () => navigateVADatum(1));

    // Aktualisieren
    bindButton('btnReq', requeryAll);

    // Auftragsliste Filter
    bindButtonAliases(['btnAbWann', 'btn_AbWann'], applyAuftraegeFilter);
    ...
    bindButtonAliases(['btnAuftragKopieren', 'Befehl640'], kopierenAuftrag);
    ...
    // Formular schließen
    bindButtonAliases(['Befehl38'], closeForm);
...
function bindFieldEvents() {
    ...
    if (veranstalter) {
        ...
        veranstalter.addEventListener('dblclick', () => {
            if (window.ConsysShell?.showForm) {
                window.ConsysShell.showForm('kundenstamm');
            }
        });
    }
...
function bindButtonAliases(ids, handler) {
    ids.forEach(id => bindButton(id, handler));
}
...
function openMitarbeiterauswahl() {
    ...
    // Wenn wir im Shell-Tab eingebettet sind, dort öffnen.
...
function openZusatzdateien() {
    ...
    // Zusatzdateien befinden sich im Attach-Tab; requery erzwingen.
    sendToSubform('sub_ZusatzDateien', { type: 'requery' });
