...
function setupEventListeners() {
    ...
    document.getElementById('Befehl12')?.addEventListener('click', loadEinsaetze);
    document.getElementById('btn_uedruck')?.addEventListener('click', () => window.print());
...
function renderCalendarStructure() {
    ...
    for (let hour = START_HOUR; hour <= END_HOUR; hour++) {
        html += `
            <tr class="time-row" data-hour="${hour}">
                <td class="time-cell">${hour.toString().padStart(2, '0')}:00</td>
                <td class="day-cell" data-day="0"></td>
                <td class="day-cell" data-day="1"></td>
                <td class="day-cell" data-day="2"></td>
                <td class="day-cell" data-day="3"></td>
                <td class="day-cell" data-day="4"></td>
                <td class="day-cell weekend" data-day="5"></td>
                <td class="day-cell weekend" data-day="6"></td>
            </tr>
        `;
    }
...
function updateCalendarHeader() {
    ...
    let html = '<tr class="calendar-header-row"><th class="calendar-header-cell time-col">Zeit</th>';
    ...
    for (let i = 0; i < 7; i++) {
        ...
        html += `
            <th class="calendar-header-cell day-col ${dayClass} ${todayClass}">
...
async function loadEinsaetze() {
    ...
    try {
        ...
        const result = await Bridge.query(`
            ...
            INNER JOIN tbl_VA_Auftragstamm a ON z.VA_ID = a.ID
            LEFT JOIN tbl_MA_Mitarbeiterstamm m ON z.MA_ID = m.ID
            WHERE z.VADatum BETWEEN #${fmt(startDate)}# AND #${fmt(endDate)}#
        ...
        state.einsaetze = (result.data || []).map(e => ({
            ...
            VA_ID: e.VA_ID,
            Datum: e.VADatum,
            Start: e.MA_Start || '08:00',
            Ende: e.MA_Ende || '16:00',
            Objekt: e.Objekt || e.Auftrag || '',
            Status: e.IstFraglich ? 'Fraglich' : 'Planung',
            MA_Soll: 0,
            MA_Ist: 0,
            Bemerkung: e.Bemerkung || '',
            MA_Name: `${e.Nachname || ''} ${e.Vorname || ''}`.trim()
        }));
...
function editEinsatz() {
    ...
    if (window.parent !== window) {
        ...
        const url = new URL(`frm_va_Auftragstamm.html?va_id=${va_id}`, window.location.href).href;
