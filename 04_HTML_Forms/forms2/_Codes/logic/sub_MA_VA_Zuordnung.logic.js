...
function handleParentMessage(event) {
    ...
    switch (data.type) {
        ...
        case 'set_column_hidden':
            // PKW/Einsatzleitung ausblenden bei bestimmten Veranstaltern
...
/**
 * MA-Lookup laden via REST-API
 */
async function loadMALookup() {
    try {
        // REST-API: /api/mitarbeiter
        const result = await Bridge.mitarbeiter.list({ aktiv: true });

        // Daten in Lookup-Format umwandeln
        ...
        console.error('[sub_MA_VA_Zuordnung] Fehler beim Laden MA-Lookup:', error);
    }
}
...
async function loadData() {
    ...
    try {
        // REST-API: /api/zuordnungen mit Filter
        const result = await Bridge.zuordnungen.list({
            va_id: state.VA_ID,
            vadatum_id: state.VADatum_ID
        });

        // API-Felder auf interne Namen mappen
        ...
        console.error('[sub_MA_VA_Zuordnung] Fehler beim Laden:', error);
...
function getRowClass(rec, idx) {
    ...
    // TODO: Status-Klassen basierend auf Rückmeldung
    return classes.join(' ');
}
...
function attachRowListeners() {
    if (state.locked) {
        // Rebuild handlers after unlocking; avoid binding when locked.
        return;
    }
...
async function handleFieldChange(event) {
    ...
    try {
        ...
        console.error('[sub_MA_VA_Zuordnung] Fehler beim Speichern:', error);
        // Wert zurücksetzen
        const rec = state.records.find(r => r.ID == id);
        if (rec) el.value = rec[field];
    }
}
...
async function handleCheckboxChange(event) {
    ...
    try {
        ...
        // Bei Einsatzleitung: Zeilen-Klasse aktualisieren
        if (field === 'Einsatzleitung' || field === 'PKW') {
...
async function addNewRow() {
    ...
    try {
        ...
        console.error('[sub_MA_VA_Zuordnung] Fehler beim Einfügen:', error);
...
function applyLockedState() {
    ...
    document.querySelectorAll('input, select, button').forEach(el => {
        if (el.id === 'new_cboMA_Ausw' || el.id === 'new_MA_Start' || el.id === 'new_MA_Ende' || el.id === 'btnAddRow') {
    ...
    const newRow = document.getElementById('newRowArea');
    if (newRow) newRow.style.display = disabled ? 'none' : '';
}
