...
async function loadData() {
    ...
    try {
        ...
        const result = await Bridge.query(`
            ...
            LEFT JOIN tbl_VA_AnzTage d ON a.VA_ID = d.VA_ID
            LEFT JOIN tbl_VA_Start s ON s.VAStart_VADatum_ID = d.VADatum_ID
            LEFT JOIN tbl_MA_VA_Planung p ON s.VAS_ID = p.VAStart_ID
            LEFT JOIN tbl_MA_Mitarbeiterstamm m ON p.MA_ID = m.MA_ID
            WHERE d.VADatum >= #${von}# AND d.VADatum <= #${bis}#
            ORDER BY a.VA_ID, d.VADatum, s.VAS_Von
        `);
